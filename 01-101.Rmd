...
The Fred Hutch cluster uses [Slurm](https://slurm.schedmd.com/overview.html) to organize and prioritize jobs. Instead of running commands directly on rhino (where you log in), you will submit a script (a snippet of directions) to tell Slurm what to do.  This work will be performed by the other networked computers in the cluster, that are collectively what people often call gizmo.  
...
Let's get a bit more advanced. We can load a preconfigured software bundle called a module. At the Fred Hutch new software modules you commonly use can be requested at any time by emailing SciComp and providing information about the software itself.  

This is very convenient because it means we don't need to install anything manually! In this example, we will load a module containing [R](https://www.r-project.org/) version 4.2.0.
