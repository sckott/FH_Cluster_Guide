...
1. Log into the Hutch VPN if needed (log in using Hutch email and CenterNet password)
1. Go to the [SciComp Self-Service Portal](https://scicomp-self-service.fredhutch.org/) (will not work if you need to log into VPN)
...
Note that it may take just a bit of time for SciComp to see your email request, but it is usually fairly quick! You will then receive an email back from the SciComp team that you now have cluster access. (The access checking page will also show that you have cluster access if you refresh the page.)
...
Now that you've logged into rhino you will be able to send scripts to the networked computers in the cluster. The Fred Hutch cluster uses [Slurm](https://slurm.schedmd.com/overview.html) to organize and prioritize jobs. Based on the instructions in your script, Slurm will find computing resources within the cluster to run your job along with all the other requests from other users.
...
`cat filename` This command prints the contents of a specific file (_filename_).

`sbatch filename.sh` This command submits a job to the cluster with instructions specified in a `.sh` file.
</div>
...
Make sure you press the download buttons that are a bit further down the screen (that say Windows or MacOS) as you may get ads for other downloads.
...
It may ask if you want to save your workspace. You can say `n` for no.
...
<div class = "dictionary">
`grabnode` This command starts an interactive session on the cluster.

**Node** One of the networked computers in the cluster. 
...
<div class = "warning">
Note that if you are working with sensitive data (such as data with [PHI](https://www.hhs.gov/answers/hipaa/what-is-phi/index.html) that requires [HIPAA compliance](https://www.hhs.gov/hipaa/index.html), you need to be extra cautious about transferring your data to the cluster. Your home directory is not an appropriate storage option for such data. Stay tuned for more resources about how to safely work with such data on the cluster.
</div>
